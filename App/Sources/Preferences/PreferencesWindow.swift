...
final class PreferencesWindow: NSObject, NSWindowDelegate {
    ...
    func show(tab: PreferencesTab? = nil) {
        // LSUIElement apps stay out of the Dock by default. Promote to a
        // regular app while Settings is open so the Capso icon appears there
        // (and Cmd-Tab), then drop back to accessory on close.
        showDockIcon()

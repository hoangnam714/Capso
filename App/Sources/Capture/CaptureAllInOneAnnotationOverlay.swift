...
final class CaptureAllInOneAnnotationOverlay {
    ...
    /// Parent lowers capture chrome then raises annotate.
    var onRequestFront: (() -> Void)?
    ...
    /// Handles ⌘C / ⌘V / ⌘D for annotation objects. Returns `true` when an
    /// object clipboard action was performed (so callers can skip image copy).
    @discardableResult
    func handleClipboardShortcut(_ event: NSEvent) -> Bool {
        guard let session, !session.isEditingText else { return false }
    ...
    /// Raises annotate chrome above sibling capture panels.
    /// Prefer calling via the parent so capture chrome can be lowered first.
    func orderFront() {
    ...
    /// Lowers annotate chrome so the capture side rail can sit above it.
    func orderBack() {
        setChromeRaised(false)
    }

    private func setChromeRaised(_ raised: Bool) {
        // Level beats same-level `orderFront` for `.nonactivatingPanel` siblings.
        let base = NSWindow.Level.screenSaver.rawValue + 3
        let front = NSWindow.Level.screenSaver.rawValue + 4
        toolbarWindow?.level = NSWindow.Level(rawValue: raised ? front : base)
    }
    ...
    private func showCanvas(session: AllInOneAnnotationSession, selectionRect: CGRect) {
        ...
        panel.onMouseDown = { [weak self] in
            if let onRequestFront = self?.onRequestFront {
                onRequestFront()
            } else {
                self?.orderFront()
            }
    ...
    private func showToolbar(session: AllInOneAnnotationSession, selectionRect: CGRect, avoidingFrame: CGRect?) {
        ...
        // Starts raised when annotate opens; capture chrome click lowers this
        // back to base so the side rail can take the front level.
        panel.level = .screenSaver + 4
        panel.isOpaque = false
        panel.backgroundColor = .clear
        panel.hasShadow = false
        panel.hidesOnDeactivate = false
    ...
    private func toolbarFrame(for selectionRect: CGRect, avoidingFrame: CGRect?) -> CGRect {
        ...
        // Prefer a vertically free, horizontally centered slot. Keep centered
        // even if it overlaps the side rail — click-to-raise switches which
        // same-level panel is frontmost.
        for candidate in candidates {
            if !candidate.intersects(inflatedAvoidance) {
                return candidate
            }
        }

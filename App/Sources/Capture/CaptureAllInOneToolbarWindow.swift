...
final class CaptureAllInOneToolbarWindow {
    ...
    private func showToolbar() {
        ...
        panel.onMouseDown = { [weak self] in
            self?.bringCaptureChromeToFront()
        }
    ...
    private func showAnnotationOverlayIfPossible() {
        ...
        overlay.onRequestFront = { [weak self] in
            self?.bringAnnotationChromeToFront()
        }
    ...
    /// Raises capture side rail / toolbar above annotate chrome.
    private func bringCaptureChromeToFront() {
        annotationOverlay?.orderBack()
        toolbarWindow?.level = .screenSaver + 4
        toolbarWindow?.orderFrontRegardless()
        toolbarWindow?.makeKey()
    }

    private func bringAnnotationChromeToFront() {
        toolbarWindow?.level = .screenSaver + 3
        annotationOverlay?.orderFront()
    }
    ...
    private func handleKeyboardEvent(_ event: NSEvent) -> NSEvent? {
        ...
        if annotationOverlay?.handleClipboardShortcut(event) == true {
            return nil
        }
